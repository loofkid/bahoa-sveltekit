...
const IntersectionOptions = new WeakMap();
...
class IntersectionBasedOffset {
  ...
  set rootMargin(value) {
    ...
    for (let i = 0; i < margins.length; i++) {
      const parsedValue = parseLength(margins[i], true);
      if (!parsedValue) throw TypeError("Unrecognized rootMargin length");
      const positions = MARGIN_MAP[margins.length - 1][i];
      for (let j = 0; j < positions.length; j++) {
        parsedMargins[positions[j]] = [
          parseFloat((<CSSUnitValue>parsedValue).value.toString()),
          (<CSSUnitValue>parsedValue).unit,
        ];
      }
    }
...
export function calculateOffset(source, orientation, offset, startOrEnd) {
  ...
  const parsedMargins = IntersectionOptions.get(offset).rootMargin;
  ...
  const clamped = IntersectionOptions.get(offset).clamp;
  const target = offset.target.getBoundingClientRect();
