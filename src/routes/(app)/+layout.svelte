<script lang="ts">
    ...
    data.firebaseAuth.onAuthStateChanged(async (firebaseUser) => {
        data = {...data};
        if (!firebaseUser) {
    ...
    $: {
        if ($userAuthStore?.photoURL !== null && $userAuthStore?.displayName !== null) {
            data = {...data};
        }
    }
...
<div class="contents" style="--header-height:{toPx('8rem')}px;">
    <div class="grid [grid-template-rows:_var(--header-height)_auto] overflow-x-hidden [grid-column:1] [grid-row:1]">
        <div class="h-[var(--header-height)] bg-slate-400 dark:bg-slate-600 grid px-2 [grid-template-columns:_1fr_1fr] [grid-template-rows:_1fr] justify-between items-center z-[1000]">
